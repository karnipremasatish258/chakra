...
const Videos = () => {
  ...
  return (
    <Stack direction={['column', 'row']} h={'100vh'} p={'20'}>
      <VStack w={'full'}>
            <video src={videoSrc}
            controls
            ...
            <VStack alignItems={'flex-start'} w={'full'}>
                  ...
                  <Text overflowY={'auto'}>This is a samlpe video for testing and demo. this is called description</Text>
            </VStack>
      </VStack>
      <VStack w={['full','xl']} alignItems={'stretch'} paddingLeft={'8'} spacing={'8'} overflowY={'auto'}>
            {
                  videosArr.map((item, index) => (
                  <Button onClick={()=> setVideoSrc(item)} variant={'ghost'} colorScheme={'purple'}>Lecture {index + 1}</Button>)
                  )
            }
      </VStack>
    </Stack>
